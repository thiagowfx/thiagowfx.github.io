...
This is a pretty much standard cmake build. If your system is missing some of
its dependencies, you will be warned and you should install them. Then:

```shell
make OgreDoc install
```

Now it is a good moment to read [xkcd](https://xkcd.com/303/). The usual
difference here is that we don't do a normal **make install**. Why? Because we
don't want our stuff ending up on **/usr/local**. Instead, we want it somewhere
where we have full access as a normal (non-root) user – in this example,
**$HOME/.lib/ogre**.

## Basic 1

Now, head to the [basic
1](http://www.ogre3d.org/tikiwiki/tiki-index.php?page=Basic+Tutorial+1) page of
the ogre wiki and download its example. You should try to build it, but take
care to change all ogre3d references from /usr/local to your new install
directory ($HOME/.lib/ogre). Yeah, that's it. **Note:** if the _basic 1_
download doesn't come with a CMakeLists.txt file, you can download it from [the
Ogre CMake wiki
page](http://www.ogre3d.org/tikiwiki/tiki-index.php?page=Building+Your+Projects+With+CMake)
(clean ogre cmake project). You can use it verbatim; just put it in the same
directory as the basic 1 files **and** – again – change any references to
_/usr/local_ to your new install directory. Also, copy the **dist/** directory
too (if it is missing from the Basic 1 project), I'll add a small observation:
in the case cmake complains about pkg-config, try adding this line to the top of
your CMakeLists.txt (but after the project version):
